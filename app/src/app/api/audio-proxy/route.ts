...
// Allowlist of domains we proxy audio from (Suno CDN)
const ALLOWED_HOSTS = [
  'cdn1.suno.ai',
  'cdn2.suno.ai',
  'cdn.suno.ai',
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const responseHeaders: Record<string, string> = {
      ...
      'Cache-Control': 'public, max-age=86400', // Cache for 24h (stems don't change)
    ...
    // Use the body stream directly to avoid buffering large audio files in memory
    if (res.body) {
      return new NextResponse(res.body, { headers: responseHeaders });
    }

    // Fallback: buffer if no readable stream (shouldn't happen in modern runtimes)
