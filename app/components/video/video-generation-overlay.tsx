...
import type { VideoGenerationPhase } from "@/lib/layertune-types";
...
const PHASE_MESSAGES: Record<string, { label: string; sub: string }> = {
  parsing_lyrics: {
    ...
    sub: "Breaking down your song structure...",
  },
  generating_backgrounds: {
    label: "Creating backgrounds",
    sub: "DALL-E is painting your scenes...",
  },
  uploading_assets: {
    label: "Uploading assets",
    sub: "Sending your photo and audio to HeyGen...",
  },
  generating_video: {
    label: "Generating video",
    sub: "HeyGen is bringing your avatar to life...",
  },
  polling: {
    label: "Processing",
    sub: "Your music video is being rendered...",
  },
  complete: {
    label: "Ready",
    sub: "Your music video is ready!",
  },
  error: {
    label: "Error",
...
const STEP_LABELS: Record<string, string> = {
  parsing_lyrics: "Lyrics",
  generating_backgrounds: "Scenes",
  uploading_assets: "Upload",
  generating_video: "Generate",
  polling: "Render",
};

function ProgressBar({ phase }: { phase: VideoGenerationPhase }) {
  ...
  const phaseProgress = Math.min(
    0.95,
    1 - Math.exp(-elapsed / (estimate * 0.5))
  );
  // Overall progress: combine completed phases + current phase progress
  const overallProgress =
    ((Math.max(0, currentIdx) + phaseProgress) / STEPS.length) * 100;
...
export function VideoGenerationOverlay({
  ...
  const isActive =
    phase === "parsing_lyrics" ||
    phase === "generating_backgrounds" ||
  ...
  const prevPhaseRef = useRef<VideoGenerationPhase>("idle");
  ...
  useEffect(() => {
    if (isActive) {
      ...
      // 600ms exit animation on complete
      setExiting(true);
  ...
  const msg =
    ...
    (exiting
      ? { label: "Ready", sub: "Your music video is ready!" }
